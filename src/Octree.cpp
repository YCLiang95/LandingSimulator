...
void Octree::create(const ofMesh & geo, int numLevels) {
	...
	int t = geo.getNumVertices();
	for (int i = 0; i < t; i++)
		root.points.push_back(i);
	subdivide(geo, root, numLevels, 1);
}

void Octree::subdivide(const ofMesh & mesh, TreeNode & node, int numLevels, int level) {
	vector<Box> box_list = vector<Box>();
	subDivideBox8(node.box, box_list);
	for (int i = 0; i < 8; i++) {
		TreeNode t = TreeNode();
		t.box = Box(box_list[i]);
		t.points.clear();
		if (getMeshPointsInBox(mesh, node.points, t.box, t.points) > 0) {
			if (level < numLevels && t.points.size() > 1)
				subdivide(mesh, t, numLevels, level + 1);
...
bool Octree::intersect(const Ray &ray, const TreeNode & node, TreeNode & nodeRtn) {
	...
	if (node.box.intersect(ray, -1000, 1000)) {
		if (node.children.size() == 0) {
			...
			for (int i = 0; i < node.children.size(); i++)
				if (intersect(ray, node.children[i], nodeRtn)) return true;
			return false;
		}
	} else
		return false;
